...
class BotsortTracker(Tracker):
    ...
    # a subset of fields.
    _DEFAULT_BOTSORT_ARGS: dict[str, Any] = {
    ...
    def __init__(self, get_current_config: GetCurrentConfig) -> None:
        ...
        self._track_state: dict[TrackId, dict[str, Any]] = {}
    ...
    def _build_args(self) -> Any:
        ...
        args_dict.update(self.config.botsort.tracker_params)
    ...
    def _ensure_botsort_initialized(self, frame: DetectedFrame) -> Any:
        ...
        self._botsort = BOTSORT(args=self._build_args(), frame_rate=30)
        return self._botsort
    ...
    def _build_ultralytics_results(self, frame: DetectedFrame) -> Any:
        ...
        return _Results(conf=conf, xywh=xywh, cls=cls)
    ...
    def track_frame(
        ...
        tracks = botsort.update(results, frame.image, None)
        ...
        for det_idx, det in enumerate(frame.detections):
            ...
            if ot_id in self._track_state:
                # If it exists here, it is either newly created on this frame or
                # already observed earlier. We can determine by first_frame match.
                is_first = int(self._track_state[ot_id]["first_frame"]) == frame.no
            tracked_detections.append(det.of_track(ot_id, is_first))
