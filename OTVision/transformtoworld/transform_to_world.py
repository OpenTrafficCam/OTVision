...
Hence, from "refpts_world" only the three digits before and all digits after delimiter are used for transformation.
After transformation the truncated thousands digits are concatenated again.
To avoid errors due to different thousands digits within study area, before transformation "refpts_world" are shifted,
so that the center of all reference points lies on a round 500 m value for both x & y UTM coordinates.
After transformation the "traj_world" coordinates are shifted back by exactly the same value.
...
def calculate_homography_matrix(refpts_pixel, refpts_world):
    ...
    refpts_world_upshifted_predecimal_pt1 = np.divide(refpts_world_upshifted_predecimal, 1000).astype(int)
    refpts_world_upshifted_predecimal_pt1_1row = np.array([[refpts_world_upshifted_predecimal_pt1.item(0), refpts_world_upshifted_predecimal_pt1.item(1)]])
    refpts_world_upshifted_predecimal_pt2 = np.mod(refpts_world_upshifted_predecimal, 1000)
    refpts_world_upshifted_disassembled = np.add(refpts_world_upshifted_predecimal_pt2, refpts_world_upshifted_postdecimal)

    # Calculate homography matrix with refpts in pixel coordinates and truncated & shifted refpts in world coordinates
    homography_matrix, mask = cv2.findHomography(refpts_pixel, refpts_world_upshifted_disassembled, cv2.RANSAC, 3.0)  # RANSAC: Otulier/Inlier definieren??? # FEHLER:
...
def convertPixelToWorld(traj_pixel, homography_matrix, refpts_world_upshifted_predecimal_pt1_1row, upshift_world):
    ...
    traj_world_upshifted_np_disassembled_3d = cv2.perspectiveTransform(traj_pixel_np_tmp, homography_matrix)
    traj_world_upshifted_np_disassembled = np.squeeze(traj_world_upshifted_np_disassembled_3d)

    # Concatenate the thousands digits truncated before transformation
    traj_world_upshifted_np = np.add(np.multiply(refpts_world_upshifted_predecimal_pt1_1row, 1000),
...
def main():
    ...
    homography_matrix, refpts_world_upshifted_predecimal_pt1_1row, upshift_world = calculate_homography_matrix(refpts_pixel, refpts_world)
    ...
    for traj_pixel_path in traj_pixel_paths:
        ...
        traj_world = convertPixelToWorld(traj_pixel, homography_matrix, refpts_world_upshifted_predecimal_pt1_1row, upshift_world)
